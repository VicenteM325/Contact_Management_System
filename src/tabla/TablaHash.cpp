...
void TablaHash::mostrarContactosEnGrupos() {
    for (int i = 0; i < capacidad; ++i) {
        for (Grupo* grupo : tabla[i]) {
            std::cout << "Grupo: " << grupo->nombre << std::endl;
            grupo->mostrarContactos(); // Llama al método para mostrar los contactos del grupo
            std::cout << std::endl;
        }
    }
}
